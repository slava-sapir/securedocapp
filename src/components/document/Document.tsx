          ...
          <div className="col-lg-5">
            <div className="candidate-list-content mt-3 mt-lg-2">
              <h5 className="fs-19 mb-0">
                ...
                <span className="badge bg-info"><i className="bi bi-database"></i> {document.formattedSize}</span>
              </h5>
              <p className="text-muted mb-2">Owner:  {document.ownerName}</p>
              <ul className="list-inline mb-0 text-muted">
                <li className="list-inline-item"><i className="mdi mdi-map-marker"></i> Created: {new Intl.DateTimeFormat('en-US', { weekday: 'long', year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(new Date(document.createdAt))}</li>
              </ul>
            </div>
          </div>
          <div className="col-lg-4">
            <div className="mt-2 mt-lg-0 d-flex flex-wrap align-items-start gap-1">
              <span className="badge bg-soft-secondary fs-14 mt-1">Type: {document.extension.toUpperCase()}</span>
