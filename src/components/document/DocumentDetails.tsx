                ...
                <div className="row align-items-center">
                  <div className="col-md-3">
                    <div className="text-center border-end">
                      <img src={documentData.data.documents.icon} className="avatar-xxl" alt={documentData.data.documents.name} />
                    </div>
                  </div>
                  <div className="col-md-9">
                    <div className="ms-3 text-lg-start text-sm-center text-xs-center">
                      <h4 className="card-title mb-2 mt-sm-3">{documentData.data.documents.name}</h4>
                      <div className="row mt-3">
                        <div className="col-md-12">
                          <button type="button" onClick={() => onDownloadDocument(documentData.data.documents.name)} className="btn btn-primary downloadb">
                            <i className="bi bi-download"></i> Download
                          </button>
                          {userData.data.user.authorities.includes('document:delete') && <button type="button" className="btn btn-danger"><i className="bi bi-trash"></i> Delete</button>}
                        </div>
                      </div>
                    </div>
                  </div>
                      ...
                      <form onSubmit={handleSubmit(onUpdateDocument)} className="needs-validation" noValidate>
                        ...
                        <div className="row g-3">
                          <div className="col-sm-6">
                            <input type="hidden" {...register('documentId')} name='documentId' className='disabled' defaultValue={documentData.data.documents.documentId} required />
                            <label htmlFor="firstName" className="form-label">Name</label><div className="input-group has-validation">
                              ...
                              <input type="text" {...register('name')} name='name' className={`form-control ' ${form.errors.name ? 'is-invalid' : ''} ${isFieldValid('name') ? 'is-valid' : ''}`} placeholder="Document name" defaultValue={documentData.data.documents.name} required disabled={!userData.data.user.authorities.includes('document:update')} />
                          ...
                          <div className="col-sm-6">
                            ...
                            <div className="input-group has-validation">
                              ...
                              <input type="text" {...register('formattedSize')} name='size' className="form-control disabled" defaultValue={documentData.data.documents.formattedSize} placeholder="Size" required readOnly />
                          ...
                          <div className="col-12">
                            ...
                            <div className="input-group has-validation">
                              ...
                              <input type="text" {...register('updaterName')} className="form-control disabled" defaultValue={documentData.data.documents.updaterName} placeholder="updaterName" required readOnly />
                              <div className="">{form.errors?.name?.message}</div>
                            </div>
                          </div>
                          <div className="col-sm-6">
                            <label htmlFor="firstName" className="form-label">Created At</label><div className="input-group has-validation">
                              ...
                              <input type="text" {...register('createdAt')} defaultValue={new Intl.DateTimeFormat('en-US', { weekday: 'long', year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(new Date(documentData.data.documents.createdAt))} className="form-control disabled" placeholder="Document name" required readOnly />
                              <div className="">{form.errors?.name?.message}</div>
                            </div>
                          </div>
                          <div className="col-sm-6">
                            ...
                            <div className="input-group has-validation">
                              <span className="input-group-text"><i className="bi bi-calendar"></i></span>
                              <input type="text" {...register('updatedAt')} defaultValue={new Intl.DateTimeFormat('en-US', { weekday: 'long', year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(new Date(documentData.data.documents.updatedAt))} className="form-control disabled" placeholder="Size" required readOnly />
                              <div className="">{form.errors?.name?.message}</div>
                            </div>
                          </div>
                          <div className="col-12">
                            ...
                            <div className="input-group has-validation">
                              ...
                              <input type="text" {...register("uri")} name='uri' defaultValue={documentData.data.documents.uri} className="form-control disabled" placeholder="URI" required readOnly />
                              <div className="">{form.errors?.name?.message}</div>
                            </div>
                          </div>
                          <div className="col-12">
                            <label htmlFor="description" className="form-label">Description</label>
                            <textarea {...register('description')} name='description' defaultValue={documentData.data.documents.description} className={`form-control ' ${form.errors.description ? 'is-invalid' : ''} ${isFieldValid('description') ? 'is-valid' : ''}`} placeholder="Description" rows={3} required disabled={!userData.data.user.authorities.includes('document:update')}></textarea>
            ...
            <div className="card">
              <div className="card-body">
                <div className="pb-2">
                  ...
                  <hr />
                  <p>{documentData.data.documents.description}</p>
                </div>
              </div>
            </div>
            <div className="card">
                    ...
                    <table className="table table-bordered mb-0">
                      <tbody>
                        <tr>
                          ...
                          <td>{documentData.data.documents.ownerName}</td>
                        </tr>
                        <tr>
                          <th scope="row">Email</th>
                          <td>{documentData.data.documents.ownerEmail}</td>
                        </tr>
                        <tr>
                          <th scope="row">Phone</th>
                          <td>{documentData.data.documents.ownerPhone}</td>
                        </tr>
                        <tr>
                          <th scope="row">Last Login</th>
                          <td>{new Intl.DateTimeFormat('en-US', { weekday: 'long', year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(new Date(documentData.data.documents.createdAt))}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
